...
\todo[inline]{signal server technology}
CENT connects the amplifier to the interface via the brain-computer-interface (BCI) signal acquisition system OpenVibe. OpenVibe is an open-source, graphical programming environment\dots

% modular, extensible
\todo[inline]{modular, extensible}
The core software defines a workflow for NFB sessions, but can be extended by three means (short of actually adapting the source):
\begin{enumerate}
	\item \textit{plugins}
	
	\item \textit{OV scenarios}
	
	\item \textit{menu functions}
